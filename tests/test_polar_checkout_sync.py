...
class PolarCheckoutSyncTests(unittest.TestCase):
    ...
    def test_slugify_name_normalizes_human_titles(self) -> None:
        self.assertEqual(slugify_name("API Spec Validator — Quick Check"), "api-spec-validator-quick-check")
        self.assertEqual(slugify_name("  Cron Expression Builder  "), "cron-expression-builder")

    def test_product_needs_update_when_remote_price_or_visibility_drift_exists(self) -> None:
        local = type("Local", (), {
            "name": "Tool X",
            "description": "Desc",
            "price_cents": 1900,
        ...
        remote = {
            ...
            "description": "Desc",
            "visibility": "private",
            "prices": [{"price_amount": 900, "price_currency": "usd", "is_archived": False}],
        }
    ...
    def test_load_local_products_reselects_polar_session_urls_for_link_rollout(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
